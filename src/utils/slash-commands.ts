...
export class SlashCommandProcessor {
  ...
  processSlashCommand(input: string): SlashCommandResult {
    ...
    let namespace = '';
    let commandName = fullCommandName;
    if (fullCommandName.includes(':')) {
      const [ns, cmd] = fullCommandName.split(':', 2);
      namespace = ns;
      commandName = cmd;
    }

    // Find command file
    const commandFile = this.findCommandFile(commandName, namespace);
    if (!commandFile) {
      const availableCommands = this.getAvailableCommands();
      return {
        success: false,
        error: `Slash command '/${fullCommandName}' not found.\nAvailable commands: ${availableCommands.join(', ')}\nCreate .claude/commands/${namespace ? namespace + '/' : ''}${commandName}.md to define this command.`,
        availableCommands
      };
    }
  ...
  /**
   * Find command file by name and optional namespace, checking various locations
   */
  private findCommandFile(commandName: string, namespace: string = ''): string | null {
    ...
    // If namespace is provided, look in specific subdirectory
    if (namespace) {
      const namespacedPath = path.join(commandsDir, namespace, `${commandName}.md`);
      if (fs.existsSync(namespacedPath)) {
        return namespacedPath;
      }
    }

    // Direct file match in root commands dir
    const directPath = path.join(commandsDir, `${commandName}.md`);
    if (fs.existsSync(directPath)) {
      return directPath;
    }

    // Check subdirectories recursively (fallback for old format)
  ...
  private collectCommandsFromDirectory(dir: string, commands: string[], prefix: string): void {
    ...
    for (const item of items) {
      ...
      if (stat.isFile() && item.endsWith('.md')) {
        ...
        // Recurse with updated prefix using : separator
  ...
  getCommandSuggestions(): CommandSuggestion[] {
    ...
    try {
      this.collectSuggestionsFromDirectory(commandsDir, suggestions, '');
    } catch (error) {
      // Ignore errors
    }
  ...
  /**
   * Recursively collect command suggestions from directory
   */
  private collectSuggestionsFromDirectory(dir: string, suggestions: CommandSuggestion[], prefix: string): void {
    const items = fs.readdirSync(dir);
    ...
    for (const item of items) {
      ...
      if (stat.isFile() && item.endsWith('.md')) {
        ...
        const description = this.extractDescription(fullPath);
        suggestions.push({
          command: `/${fullCommand}`,
          description: description || `${fullCommand} command`
        });
      } else if (stat.isDirectory()) {
        // Recurse with updated prefix using : separator
        this.collectSuggestionsFromDirectory(fullPath, suggestions, `${prefix}${item}:`);
      }
    }
  }
  ...
  private extractDescription(filePath: string): string {
    try {
      ...
      // Try to find description after **
      for (const line of lines) {
        if (line.startsWith('**') && line.includes('**:') && !line.includes('USAGE NOTE')) {
      ...
      return lines[0] || 'Command template';
    } catch (error) {
