...
function ChatInterfaceWithAgent({
  ...
  useEffect(() => {
    const processedInitialMessage = preprocessPrompt(initialMessage);
    if (processedInitialMessage && agent) {
      const userEntry: ChatEntry = {
        type: "user",
        content: processedInitialMessage,
        timestamp: new Date(),
      };
      ...
      const processInitialMessage = async () => {
        ...
        try {
          ...
          for await (const chunk of agent.processUserMessageStream(processedInitialMessage)) {
            switch (chunk.type) {
              case "content":
                if (chunk.content) {
                  if (!streamingEntry) {
